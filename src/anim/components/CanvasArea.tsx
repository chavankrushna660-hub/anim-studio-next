...
export default function CanvasArea({
  ...
  const getOrPrepareActiveObject = (coords: Point): VectorObject => {
    ...
    const visibleObjs = Object.values(objects).filter(o => isObjInteractable(o));
  ...
  const handlePointerDown = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (activeTool === 'LIN') {
      ...
      if (hitObj) {
        ...
        // Find closest object to coords
        let minObjDist = Infinity;
        let closestObj: VectorObject | null = null;
        Object.values(objects).forEach(o => {
          if (o.isHidden || o.isLocked) return;
          const pivot = o.pivots?.[0] || { localX: 0, localY: 0 };
          const pts = getAllObjectPoints(o);
          pts.forEach(p => {
            const worldPt = localToWorld(p, o.transform, pivot);
            const d = distance(coords, worldPt);
            if (d < minObjDist) {
              minObjDist = d;
              closestObj = o;
        ...
        if (closestObj && minObjDist < (120 / zoomScale)) {
          targetObj = closestObj;
          setSelectedObjectId((closestObj as VectorObject).id);
        }
      }
    ...
    if (activeTool === 'SWAP_STUDIO' || activeTool === 'SWP' || activeTool === 'SST') {
      ...
      if (clickedObj) {
        ...
        // Clicking on empty canvas space unselects drawing
        setSelectedObjectId(null);
        setDragMode('none');
      }
      return;
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (isDrawing || activeTool === 'KNF' || activeTool === 'LQB' || activeTool === 'SCT' || activeTool === 'VST' || activeTool === 'VLB' || (activeTool === 'LSO' && lassoMode === 'pen') || isDrawing3DBone || boneStartPoint !== null || activeTool === 'PEN' || activeTool === 'BON') {
      setCurrentCursorPos(coords);
    }
    ...
    if (selectedObjectId) {
      ...
      if (dragMode === 'move') {
        ...
        setObjects(prev => {
          ...
          const rootSnapshot = snapshotMap.get(selectedObjectId) || initialTransform || activeObj.transform;
  ...
  useEffect(() => {
    ...
    sortedObjects.forEach((obj) => {
      ...
      if (drawObj.type === 'image' && drawObj.imageUrl) {
        ...
        if (img.complete && img.naturalWidth > 0) {
          ...
          if (hasLassoDeformImage || hasVstImage || hasMeshState || hasPuppetPins || hasCageState || hasSmartWarp || hasLcp || hasVdf || hasFlexCurve || hasCurvePath) {
    ...
    if (effectiveSelectedObjectId && objects[effectiveSelectedObjectId] && !isSelLayerLocked && !isSelLayerHidden && !objects[effectiveSelectedObjectId].isLocked && !objects[effectiveSelectedObjectId].isHidden && activeTool !== 'ZOM') {
      ...
      // 1.8. Draw 3D Rule Engine Virtual Soul Gizmo & Orientation Orbit
      if (obj.rule3DState && obj.rule3DState.enabled) {
        const centerPt = localToWorld({ x: box.x + box.width / 2, y: box.y + box.height / 2 }, obj.transform, pivot);
    ...
    // Render active VST (Vector Sub-Region Transform) overlay
    if (effectiveSelectedObjectId && objects[effectiveSelectedObjectId] && (activeTool === 'VST' || objects[effectiveSelectedObjectId].vstState?.active)) {
      const obj = objects[effectiveSelectedObjectId];
      if (obj.vstState && obj.vstState.lassoPoints && obj.vstState.lassoPoints.length >= 3) {
        const localPivot = obj.pivots[0] || { localX: 0, localY: 0 };
